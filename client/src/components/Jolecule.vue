...
.w-100.d-flex.flex-column.user-select-none(style="background-color: #CCC")
  ...
  // Top bar
  .w-100.d-flex.flex-row.justify-content-between.mx-2(style="height: 40px")

      .d-flex.flex-row.flex-nowrap.align-items-center.text-nowrap
        .text-center(style="width: 1.4em; background-color: #BBB")
          .spinner-grow.spinner-grow-sm.text-primary(v-if="isLoading")
          span(v-if="!isLoading") &nbsp;
        // Title
        .ms-2 {{ title }}
  ...
  .w-100.d-flex.flex-row(style="height: calc(100vh - 40px)")
  ...
  draw() {
    ...
    for (let i = 0; i < this.nGridX; i += 1) {
      for (let j = 0; j < this.nGridY; j += 1) {
        ...
        for (let selectedIframe of this.iFrameTrajs) {
          if (isSameVec(iFrameTraj, selectedIframe)) {
            this.fillRect(
              i * this.diffX + this.diffX / 2 - boxXHalf,
              j * this.diffY + this.diffY / 2 - boxYHalf,
              boxX,
...
export default {
  ...
  async mounted() {
    ...
    this.mode = (await rpc.remote.get_config("mode"))?.result;
    this.key = (await rpc.remote.get_config("key"))?.result;
    this.opt_keys = (await rpc.remote.get_config("opt_keys"))?.result;

    await this.$forceUpdate();

    let backgroundColor = (await rpc.remote.get_config("background"))?.result;
  ...
  methods: {
    async get_config(key) {
      ...
      await this.$forceUpdate();
      let response = await rpc.remote.get_config("matrix");
    ...
    async loadMatrix(iFrameTraj) {
      ...
      console.log(`loadMatrix iFrameTraj=${iFrameTraj}`);
    ...
    async selectMatrixGridValue(value, selectOnly = false) {
      console.log(`chooseMatrixGridValue value`, _.cloneDeep(value));
      let iFrameTraj;
      if (_.has(value, "iFrameTrajs")) {
        let label = value.label;
        let n = value.iFrameTrajs.length;
        let grid = [
          _.map(value.iFrameTrajs, (iFrameTraj, i) => ({ p: i / n, label, iFrameTraj })),
        ];
      ...
      if (_.has(value, "iFrameTraj")) {
        ...
        if (!_.isNil(iFrameTraj)) {
          if (this.hasFramesInJolecule() || !inFrames(this.iFrameTrajs, iFrameTraj)) {
    ...
    async loadStrip() {
      let response = await rpc.remote.get_config("strip");
    ...
    async selectStripGridValue(value, selectOnly) {
      ...
      if (this.hasFramesInJolecule() || !inFrames(this.iFrameTrajs, iFrameTraj)) {
        console.log(`StripWidget.chooseIframe iFrameTraj=${_.cloneDeep(iFrameTraj)}`);
        await this.loadFrameIntoJolecule(iFrameTraj, selectOnly);
      }
    },
    ...
    async loadTable(iFrameTraj) {
      let response = await rpc.remote.get_config("table");
      this.table = response.result;
      if (_.isEmpty(this.table)) {
        return;
      }
      let headers = (await rpc.remote.get_config("table_headers"))?.result;
      if (headers) {
    ...
    async getPdbLines(iFrameTraj) {
      ...
      if (this.isAlphaSpace) {
        if (key in this.cacheAlphaSpaceByIframe) {
          console.log(`getPdbLines from cacheAlphaSpaceByIframe[${key}]`);
          result = this.cacheAlphaSpaceByIframe[key];
        } else {
          this.isLoading = true;
          await this.$forceUpdate();
          let response = await rpc.remote.get_pdb_lines_with_alphaspace(iFrameTraj);
          this.isLoading = false;
          if (response.result) {
            this.cacheAlphaSpaceByIframe[key] = response.result;
            result = response.result;
          }
        }
      } else {
        if (key in this.cacheByIframe) {
          console.log(`getPdbLines from cacheByIframe[${key}]`);
          result = this.cacheByIframe[key];
        } else {
          this.isLoading = true;
          await this.$forceUpdate();
          let response = await rpc.remote.get_pdb_lines(iFrameTraj);
          this.isLoading = false;
          if (response.result) {
            this.cacheByIframe[key] = response.result;
            result = response.result;
          }
        }
      }
    ...
    async selectOptKey(key) {
      ...
      await rpc.remote.select_new_key(key);
