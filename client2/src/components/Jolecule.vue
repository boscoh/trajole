...
export default {
  ...
  methods: {
    ...
    async reload() {
      ...
      this.opt_keys = (await rpc.remote.get_config(this.foamId, "opt_keys"))?.result;
      await this.$forceUpdate();
      if (this.mode === "strip") {
    ...
    async loadFoamId(foamId) {
      ...
      this.foamId = foamId
      this.title = `Connecting to FoamDB Trajectory ID=${this.foamId}`
      this.isLoading = true;
      await this.$forceUpdate()
    ...
    async selectStripGridValue(value, selectOnly) {
      ...
      if (this.hasFramesInJolecule() || !inFrames(this.iFrameTrajs, iFrameTraj)) {
        console.log(`StripWidget.chooseiFrameTraj iFrameTraj=${_.cloneDeep(iFrameTraj)}`);
        await this.loadFrameIntoJolecule(iFrameTraj, selectOnly);
      }
    },
    ...
    async loadTable(iFrameTraj) {
      ...
      let headers = (await rpc.remote.get_config(this.foamId, "table_headers"))?.result;
      if (headers) {
    ...
    async getPdbLines(iFrameTraj) {
      ...
      if (this.isAlphaSpace) {
        if (key in this.cacheAlphaSpaceByiFrameTraj) {
          ...
          this.isLoading = true;
          await this.$forceUpdate();
          let response = await rpc.remote.get_pdb_lines_with_alphaspace(this.foamId, iFrameTraj);
          this.isLoading = false;
          if (response.result) {
